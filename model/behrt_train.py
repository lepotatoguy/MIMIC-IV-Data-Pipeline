...
class train_behrt():
    def __init__(self,src, age, sex, ethni, ins, target_data):
        ...
        # Fill NaN values with 0 or handle them as needed for each column
        src = src.fillna(0)
        age = age.fillna(0)
        sex = sex.fillna(0)
        ethni = ethni.fillna(0)
        ins = ins.fillna(0)
        ...
        self.train_params = {
            'batch_size': 16,
            'use_cuda': True,
            'max_len_seq': self.global_params['max_seq_len'],
        ...
        conf = BertConfig(self.model_config)
        self.behrt = BertForEHRPrediction(conf, self.model_config['number_output'])

        self.behrt = self.behrt.to(self.train_params['device'])

        #models parameters
        transformer_vars = [i for i in self.behrt.parameters()]

        #optimizer
        self.optim_behrt = torch.optim.Adam(transformer_vars, lr=3e-5)

        TrainDset = DataLoader(train_data, max_len=self.train_params['max_len_seq'], code='code')
        trainload = torch.utils.data.DataLoader(dataset=TrainDset, batch_size=self.train_params['batch_size'], shuffle=True)
        ValDset = DataLoader(val_data, max_len=self.train_params['max_len_seq'], code='code')
        valload = torch.utils.data.DataLoader(dataset=ValDset, batch_size=self.train_params['batch_size'], shuffle=True)

        train_loss, val_loss = self.train(trainload, valload, self.train_params['device'])

        # Loading the model after training
        self.behrt.load_state_dict(torch.load("./saved_models/checkpoint/behrt", map_location=self.train_params['device']))
        print("Loading successful")


        TestDset = DataLoader(test_data, max_len=self.train_params['max_len_seq'], code='code')
        testload = torch.utils.data.DataLoader(dataset=TestDset, batch_size=self.train_params['batch_size'], shuffle=True)
        # loss, cost, pred, label = eval(testload, True, train_params['device'])

        # Evaluation
        loss, cost, pred, label = self.eval(testload, True, self.train_params['device'])
    ...
    def train(self, trainload, valload, device):
        ...
        for e in range(self.train_params["epochs"]):
            print("Epoch", e)
            train_loss, train_time_cost = self.run_epoch(e, trainload, device)
    ...
    def run_epoch(self, e, trainload, device):
        ...
        for step, batch in enumerate(trainload):
            ...
            logits = self.behrt(input_ids, age_ids, gender_ids, ethni_ids, ins_ids, segment_ids, posi_ids, attention_mask=attMask)
    ...
    def eval(self, valload, saving, device):
        tr_loss = 0
        start = time.time()
        self.behrt.eval()
        if saving:
